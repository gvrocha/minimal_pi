...
- [ ] **Wire AP config into milestone 1's script(s) as a second network-mode option** — apkovl needs a wlan0 stanza + hostapd/dnsmasq service enablement + package list addition (`build-apkovl.sh`), and `flash-and-install.sh` needs a mode selector, since its current CLI is Ethernet/direct-cable-specific end to end (Mac-side `networksetup` interface config, default-route-hijack fix, `MAC_SERVICE` argument) with no AP-mode equivalent yet.
- [ ] **Live end-to-end AP test on real Pi 3B+ hardware** — flash a card in AP mode, connect a client with no Ethernet involved, confirm SSH access. Bring-up order (interface+IP before hostapd before dnsmasq) and actual brcmfmac AP-mode association behavior are both still unverified — treat as genuinely open, not a formality, given the sibling project's own AP attempt was never confirmed working.
...
**Milestone 1 is done (2026-08-01).** Milestone 2: hostapd.conf/dnsmasq.conf design is done (2026-08-02, `alpine/wifi-ap/`). Remaining: wire AP mode into `build-apkovl.sh`/`build-answerfile.sh`/`flash-and-install.sh` as a second network-mode option, then a live end-to-end AP test on real hardware. See `alpine/lessons-learned.md`'s "Not yet exercised" section for starting context.
